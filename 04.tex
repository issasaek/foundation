...
\kagi{$
    \app{\epsilon} \delta
$}は\kagi{$
    \arg\delta\cap\classab{x:\iter{\mathrm{E}}{2}\fap(x\ast\trgl{\delta})\in\mathrm{Cseq}^\epsilon}
$}を表わす．
\end{df}
...
$x\in\app{\epsilon}\delta$について，$ \iter{\mathrm{E}}{2}\fap (x\ast\trgl{\delta})\in\mathrm{Cseq}^\epsilon $が実現しているとき，$x$は$\delta$の構成要件$\exe{\epsilon}\delta$に属する．
...
次に，$ \tildel{\trgl{\delta}}\fap 0 $は$\delta$の修正条件と言い，（制御可能性において）制御方向の違いに応じて構成要件の実現を促進する／抑制する因子となる．そして，$ x\in\arg\delta $について，$\tilde{x}\fap 0$はそれについて修正条件が実現すべき対象である．
修正条件は規制類型ごとに異なるが，
\[
...
$}は\kagi{$
    \classab{x:
        (\exists n)(\Lambda\neq n\subseteq\mathfrak{h}\epsilon\con{1}
        \orp{x,\exe{\epsilon}\delta}\to_{\epsilon}\orp{\tilde{x}\fap 0,\prob{(\tildel{\trgl{\delta}}\fap 0)}{n\uphl\epsilon}}
        )
    }
$}を表わす．
\end{df}
...
\begin{align*}
    x\in\cs{\epsilon}\delta\con{1}x\in\app{\epsilon} \delta & \case{1}{1}{1}(x\ast\trgl{\delta})\fap 0\in\mathfrak{E}\\
    &\:\, \case{1}{0}{1}(\iter{\mathrm{E}}{2}\fap(x\ast\trgl{\delta}))\img\univ\subseteq \mathfrak{E}.
\end{align*}
つまり，$ x\in\app{\epsilon}\delta $によって，$ \iter{\mathrm{E}}{2}\fap(x\ast\trgl{\delta}) $が実現される（促進的または正の制御構造）．

他方，$ \tildel{\trgl{\delta}}\fap 1 = \barl{(\trgl{\delta}\fap 0)}\cap\msec{(\arg\delta)}{0} $の場合（$ \delta $がS類型），
\begin{align*}
    x\in\cs{\epsilon}\delta\con{1}x\in\app{\epsilon} \delta & \case{1}{1}{1}(x\ast\trgl{\delta})\fap 0\in\barl{\mathfrak{E}}\\
    &\:\, \case{1}{0}{1}(\iter{\mathrm{E}}{2}\fap(x\ast\trgl{\delta}))\img\univ\subseteq\barl{\mathfrak{E}}.
\end{align*}
つまり，$ x\in\app{\epsilon}\delta $によって，$ \iter{\mathrm{E}}{2}\fap(x\ast\trgl{\delta}) $の実現が阻止される（抑制的または負の制御構造）\footnote{
...
とみなすことができるケースがある\footnote{
    有意レベルの蓋然性を認めるためには，$ \app{\epsilon}\delta $の部分条件のうち，どれが$y_1$において保存されていなければならないかという問題がある．すなわち，どの$ z\in\classab{z:\app{\epsilon}\delta\subseteq z} $について，$ y_1\subseteq z $であるかという問題．この点，少なくとも，結節点への行動帰属の条件や，因果系列の構成についての条件$ \classab{x:\iter{\mathrm{E}}{2}\fap(x\ast\trgl{\delta})\in\mathrm{Cseq}^\epsilon} $が保存されている必要があると思われる．
}．するとこの場合，
\begin{align*}
...
この点について，\ref{ssec:執行可能性}における規制類型$\delta$の例を再び用いよう．すなわち，$ x\in\exe{\epsilon}\delta $は人の死の惹起であり，$ \trgl{\delta}\fap 0 = \mathfrak{E}\resl\sigma $は\ref{ssec:執行可能性}で示した通りの刑罰類型の系列である．また，
\[
...
$}は\kagi{$
    ...
    \\\hfill
    \arg w = \arg y\con{1}
    (\iter{\mathrm{E}}{2}\fap\trgl{w})\uphr\bar{1}\subseteq\lambda_n(\barl{(\trgl{y}\fap i)}\cap\msec{(\arg y)}{n})\con{1}
...
$ \orp{x,\app{\epsilon} y}\to_{\epsilon}\orp{z,\app{\epsilon} w} $であり得る．また，$ (z\ast\trgl{w})\fap 0 $と$ (z\ast\trgl{w})\fap 1 $との間に，
\[
...
\begin{enumerate}[label=(\arabic*)]
    ...
    \[
        \iter{\mathrm{E}}{2}\fap(x\ast\trgl{\delta}) = \iter{\mathrm{E}}{2}\fap(z\ast\trgl{y})
