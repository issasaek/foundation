...
\]
したがって，$ \mathcal{W}\fap 2\subseteq\trgl{\arg\epsilon} $である限り，\kagi{$ \in $}を$ \mathbf{F}^2_0 $に置き換えた\ref{ssec:集合論の体系}のA \ref{axim:外延性}〜A \ref{axim:分出}が，したがって，自然科学に必要な全ての数学的真理が$m_1,m_2$において真となる．
...
述語を追加する方法は\ref{ssec:原初的言語}におけるのと同様である．つまり，$\mathfrak{L}$の述語として使用する述語記号を，以下の例のような文脈的定義によって指定する．述語記号$\orp{2,0}$は既に\kagi{$ \in $}として使用されている．
\setcounter{equation}{0}
\begin{multline}
    \text{
        \kagi{$\alpha$は人間である}の\kagi{$\alpha$}に任意の変項を代入した結果は，
    }\\
    \text{\kagi{$ (F,)\alpha\beta $}に同一の代入をした結果を表わす．}
\end{multline}
\begin{multline}
    \text{
        \kagi{$\alpha$は$\beta$を愛する}の\kagi{$\alpha$}と\kagi{$\beta$}に任意の変項を代入した結果は，
       }\\
    \text{\kagi{$ (F,\!,)'\alpha\beta $}に同一の代入をした結果を表わす．}
\end{multline}
...
\begin{enumerate}[label=\Alph*.]
    \item $\breve{\epsilon}\fap 0$は，$\mathfrak{L}$の標準的モデルである．
    \item $\epsilon$が与える開始点への類似性尺度は，$\mathrm{T}^{(\breve{\epsilon}\fap 0)}\cap \mathfrak{L}$の各要素が持つ改訂圧力への耐性値と相関している．
\end{enumerate}

\paragraph{条件A}モデル$\breve{\epsilon}\fap 0$は，以下の条件を充たす唯一の$m\in\mathrm{MD}$（$\mathfrak{L}$の標準的モデル）である．$\mathfrak{L}$を構成する仕方によって，この条件は，$\breve{\epsilon}\fap\Lambda$が現実を反映したモデルであることを表現している．
\begin{enumerate}[label=(\arabic*)]
    \item $\mathcal{L}\fap m = \trgl{\arg\epsilon}$．
    \item 原始的述語でない任意の述語記号$\orp{n,i}$について，$(\mathcal{R}\fap m)\fap\orp{n,i} = \Lambda$．
    \item 原始的述語である任意の述語記号$\orp{n,i}$について，以下の型式に代入して得られる等式を使用して，各述語ごとに$\mathcal{R}\fap m$が与えるクラスを指定する．
    \[
        (\mathcal{R}\fap m)\fap\orp{\alpha,\beta} = \timex{(\mathcal{L}\fap m)}{\alpha}\cap\classab{\gamma:\delta}.
    \]
    ただし，\kagi{$ \alpha $}，\kagi{$ \beta $}にはそれぞれ$n,i$を指示する数字を，\kagi{$ \delta $}には原始式$ \orp{0,\orp{n,i},k} $を代入する．また，$ k\fap i = i $とする．そして，\kagi{$ \gamma $}には，変項$\boldsymbol{v}(0),\dots,\boldsymbol{v}(\breve{\mathrm{S}}\fap n) $から作られる順序対表現を代入する．例えば，
        \begin{enumerate}
...
ところで，真理集合$ \mathrm{T}^{(\breve{\epsilon}\fap 0)}\cap \mathfrak{L} $には，\ref{ssec:集合論の体系}のA \ref{axim:外延性}〜A \ref{axim:分出}が含まれる．これらの公理の集合を$a$と置く．また，解釈空間の定義上，任意の$h\in\arg\epsilon$について，
\[
...
\paragraph{条件B}$\epsilon$が与える開始点への類似性の尺度は，$ \mathrm{T}^{(\breve{\epsilon}\fap 0)}\cap \mathfrak{L} $の各要素が改訂圧力を受ける場合（それを含む文の集合が含意する予測が観察により反駁される場合）におけるその耐性値に相関している．

すなわち，実験状況を要約するような一般化$p\in(\mathrm{T}^{(\breve{\epsilon}\fap 0)}\cap \mathfrak{L})$が観察により反駁されたと仮定した場合，整合性を回復するためには，$p$を含意する最小単位$b\subseteq (\mathrm{T}^{(\breve{\epsilon}\fap 0)}\cap \mathfrak{L})$（どの$b'\subset b$も$p$を含意しない）のメンバーのいずれかを偽とみなす必要が生じる．
